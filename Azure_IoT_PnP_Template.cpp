...
/* --- Internal Functions --- */
const int WaterSensorPin = 21;  // Pin sensor air
const int trigPin = 13;         // Pin trig untuk ultrasonik
const int echoPin = 12;          // Pin echo untuk ultrasonik
...
// Fungsi untuk membaca nilai dari sensor air
static void get_water_sensor_status(int* WaterStatus) {
  *WaterStatus = digitalRead(WaterSensorPin); // Baca status dari sensor air (digital)
}

// Fungsi untuk membuat payload telemetry
static int generate_telemetry_payload(
    uint8_t* payload_buffer,
    size_t payload_buffer_size,
    size_t* payload_buffer_length)
{
  ...
  get_water_sensor_status(&WaterStatus); // Membaca status sensor air
  ...
  // Tambahkan status sensor air
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("WaterStatus"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding WaterStatus property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, WaterStatus);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding WaterStatus property value to telemetry payload.");
